...
// this needs to be incorporated into one function for the readabiity
ErrorCode Comparison::loopGreedyApproach(std::vector<std::tuple<node *,unsigned>>& content1, std::vector<std::tuple<node *, unsigned>>& content2, std::vector<std::tuple<unsigned, unsigned>> &alignedPairs, int &diff)
{
    unsigned i = 0; unsigned j = 0;
    std::map<node *, std::vector<unsigned>> match;
    ...
    while(i < content1.size() && j < content2.size()) {
        ...
        if(this->nodeMatch(nodeOne, nodeTwo)) {
            if(nodeOne->type != nodeType::Virtual) {
                ...
                if(this->loopGreedyApproach(((virtualNode *)nodeOne)->contents, ((virtualNode *)nodeTwo)->contents, alignedPairs, diff) != ErrorCode::SUCCESS) {
            ...
            while(i < content1.size()) {
                ...
                if(node2Match->type != nodeType::Virtual && match.find(node2Match) != match.end()) {
                    ...
                    if(!found) {
                        ...
                        i++; j++;
                        aligned = true;
                        if(this->loopGreedyApproach(((virtualNode *)node2Match)->contents, ((virtualNode *)nodeTwo)->contents, alignedPairs, diff) != ErrorCode::SUCCESS) {
