...
/*
 * This is a code I took from the git repo called RQuBE (https://github.com/idea-iitd/RQuBE) as an implementation of Biermann-Feldman algorithm
 */
...
void insert(trieNode *root,vector<string> &s){
    for(long int i=0;i<s.size();i++){
        // if you didn't find s[i] in your children 
        if(root->children.find(s[i])==root->children.end()){
            trieNode *child=newNode(); // create a new node 
            if(i==s.size()-1)
                child->EOW=1; // EOW is end of word
...
void construct(vector<vector<string>>& input,int tail,vector<unordered_map<int,unordered_set<int>>>& transition_table){
    // cleaning up all the global variables
